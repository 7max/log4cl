    ...
    (dolist (level levels)
      (let ((log-macro-name (intern (format nil "LOG-~a" level)))
        ...
        (push `(defmacro ,log-macro-name (&rest args &environment env)
                 "Logs the message with the logger. See log-debug for description of arguments"
                 (log-with-level env ,level-name args))
              list)))
    `(progn
       ,@(nreverse list))))

(deflog-macros fatal error warn info user1 user2 user3 user4 
    ...
    (cond
      ...
      ((keywordp arg)
       (values (logger-name-from-symbol arg env)
               (rest args)))
      ;; can't determine auto logger name
      (t
