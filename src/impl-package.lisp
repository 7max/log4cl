...
(macrolet
    ((%define-log4cl-package ()
       (let* ((p1 (find-package :log4cl))
              ...
              tmp
              (name-and-nickname 
                (cond ((and p1 p2 (not (eq p1 p2)))
                       ;; loading directly on top of old version
                       (delete-package p1)
                       (if (package-used-by-list p2)
                           ;; can't delete log4cl-impl, so simply give
                           ;; it log4cl nickname for now, it will get
                           ;; named right on next image load
                           `(#:log4cl-impl (:nicknames #:log4cl))
                           (progn 
                             (delete-package p2)
                             `(#:log4cl (:nicknames #:log4cl-impl)))))
                      ((and p1 p2 (equal (package-name p1)
                                         (symbol-name '#:log4cl-impl))) 
                       (if (package-used-by-list p2)
                           ;; still can't delete, weird
                           `(#:log4cl-impl (:nicknames #:log4cl))
                           (progn 
                             (delete-package p2)
                             `(#:log4cl (:nicknames #:log4cl-impl)))))
                      (t `(#:log4cl (:nicknames #:log4cl-impl)))))) 
